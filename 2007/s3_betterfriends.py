...
def dfs(curr, start, end, count, found, foundcount):
    # print(curr)
    ...
    if curr == end:
        ...
        if not found:
            return dfs(curr, curr, start, count, True, foundcount)
        else:
            return "Yes " + str(count)
    ...
    if not found:
        if curr == -1 or (curr == start and count != -1):
            return "No"    
        return dfs(friends[curr], start, end, count+1, found, foundcount)
    else:
        if curr == -1 or (curr == start and foundcount != -1):
            return "No"
        return dfs(friends[curr], start, end, count, found, foundcount+1)

while True:
    checkGiver, checkReceiver = [int(i) for i in input().split()]
    if checkGiver == 0 and checkReceiver == 0:
        break
    print(dfs(checkGiver, checkGiver, checkReceiver, -1, False, -1))